...
ncname <- "weasd.2018"
ncfname <- paste(ncpath, ncname, ".nc", sep = "")
ncfname2 <- paste(ncpath, ncname, "_V2", ".nc", sep = "")
dname <- "weasd"
...
# get variable
weasdm_array <- ncvar_get(ncin,dname)
dlname <- ncatt_get(ncin, dname, "long_name")
dunits <- ncatt_get(ncin, dname, "units")
fillvalue <- ncatt_get(ncin, dname, "_FillValue")
dim(weasdm_array)
...
#### REWRITE AFTER SUBSET
w <- terra::rast("weasd/weasd.2018.nc")
w_crop <- terra::crop(
  w,
  co,
  snap = "out"
...
y <- y[which(y > e[3] & y < e[4])]
ny <- dim(y)
weasdm_array <- weasdm_array[which(x > e[1] & x < e[2]), which(y > e[3] & y < e[4]),]
lon <- lon[which(x > e[1] & x < e[2]), which(y > e[3] & y < e[4])]
lat <- lat[which(x > e[1] & x < e[2]), which(y > e[3] & y < e[4])]
####
...
# define variables also include longitude and latitude and the CRS variable
fillvalue <- 1e32
dlname <- "Daily Accumulated Snow at Surface"
weasd_def <- ncvar_def("weasd",dunits$value,list(xdim,ydim,timedim),fillvalue,dlname,prec="single")
dlname <- "Longitude of cell center"
lon_def <- ncvar_def("lon", "degrees_east", list(xdim, ydim), NULL, dlname, prec = "double")
dlname <- "Latitude of cell center"
lat_def <- ncvar_def("lat", "degrees_north", list(xdim, ydim), NULL, dlname, prec = "double")
...
# create netCDF file and put arrays
ncout <- nc_create(ncfname2,list(weasd_def,lon_def,lat_def,proj_def),force_v4=FALSE)

# put variables
ncvar_put(ncout,weasd_def,weasdm_array)
ncvar_put(ncout, lon_def, lon)
ncvar_put(ncout, lat_def, lat)
